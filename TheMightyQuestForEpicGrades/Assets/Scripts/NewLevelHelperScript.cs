...
public class NewLevelHelperScript : MonoBehaviour {
    ...
    public void FileDialogOpener() {
        ...
        if (fbd == null)
            fbd = new FolderBrowserDialog {RootFolder = Environment.SpecialFolder.Desktop, ShowNewFolderButton = false};
        ...
        if (fileNames.Length != 0)
            foreach (var file in fileNames)
                if (FileNameHelper.filePathList.Contains(PreselectedFolderPath + "\\" + file)) { //Wenn Pfad bereits vorhanden
    ...
    public void CopyFilesToLevelFolder() {
        ...
        foreach (var file in FilePathsAndStatus) {
            ...
            levelNumberToAdd++;
            Debug.Log(
                "Datei: " + file + "\n" + "Kopiert in Level-Ordner als:" + "Level_" + (levelNumberToAdd - 1) + ".txt");
        }
    }
